...
flags.DEFINE_string(
    'input', None, 'input directory to purdue dataset iRM version')
flags.DEFINE_string(
    'output', None, 'output directory to store TFRecord file(s)')
flags.DEFINE_integer('shard_size', 1000,
                     'number of samples per TFRecord shard')
        ...
        with tf.io.TFRecordWriter(shard_path) as writer:
            for sample in tqdm(samples[shard_id*FLAGS.shard_size:(shard_id+1)*FLAGS.shard_size],
                ...
                anno_path = os.path.join(
                    FLAGS.input, 'annotation', f'{sample}.txt')
                ...
                for annotation in annotations:
                    ...
                    difficulties.append(-1) # No difficulty information avail in this dataset, hard code to -1
                    x_center, y_center, w, h = [
                        float(val) for val in annotation.split(' ')[1:]]
                ...
                bbox_yxyx_n4 = tf.convert_to_tensor(
                    bbox_yxyx_n4, dtype=tf.float32)
...
def main(_argv):
    ...
    samples = [s.split('.')[0] for s in os.listdir(
        os.path.join(FLAGS.input, 'annotation'))]
